...
void counting_sort(int** matrix, int rows, int cols, int& q)
{
	if (q == 1)  
	{
		for (int p = 0; p < rows; p++)
		{
			...
			{
				if (matrix[p][i] < min) min = matrix[p][i];
			...
			for (int i = 0; i < cols; i++)
				count[matrix[p][i] - min]++;
			int index = 0;
			for (int i = 0; i < range; i++)  
			{
				while (count[i] > 0)
				{
					matrix[p][index++] = i + min;
					count[i]--;
				}
			}
...
void selectionSort(int** matrix, int rows, int cols, int& q) {
...
void insertion_sort(int** matrix, int rows, int cols, int q)
{
	for (int row = 0; row < rows; ++row)
	{
		for (int i = 1; i < cols; ++i)
		{
			int key = matrix[row][i];
			int j = i - 1;
...
void quick_sort_recursive(int arr[], int left, int right, bool order)
{
	if (left >= right) return;

	int half_el = arr[(left + right) / 2];
	int i = left;
	int j = right;
	...
	while (i <= j)
	{
		if (order)
		{
			while (arr[i] < half_el) i++;
			while (arr[j] > half_el) j--;
		}
		else
		{
			while (arr[i] > half_el) i++;
	...
	if (left < j) quick_sort_recursive(arr, left, j, order);
	if (i < right) quick_sort_recursive(arr, i, right, order);
}

void quick_sort(int** matrix, int rows, int cols, int q)
{
	bool order;
	if (q == 1)
		order = true;  
	...
	for (int row = 0; row < rows; ++row)
	{
		quick_sort_recursive(matrix[row], 0, cols - 1, order);
	}
}
