...
int main()
{
	...
	if (a == 1)
	{
		...
		long long u;
		int max = 0;
		el_num = random_size - 1;
		for (int i = 0; el_num >= 0; el_num--)
		{
			int tmp = abs(arr[el_num]);
			if (tmp > abs(max))
			{
				max = tmp;
				num_umn = el_num;
		...
		num_umn += 1;
		for (u = 1; num_umn != random_size; num_umn++)
		{
			u *= arr[num_umn];
		}
		std::cout << "Произведение элементов массива, расположенных после максимального по модулю элемента = " << u << std::endl;
	...
	else if (a == 2)
	{
		...
		if (std::cin.fail() || size < 0)
		{
			...
			std::cout << "Произведение элементов массива, расположенных после максимального по модулю элемента = 0";
